...
function xepmarket2_ajax_prepare_plugins_sync() {
    ...
    // 1. Download — use cached temp dir if still valid (avoid re-downloading on retry)
    require_once ABSPATH . 'wp-admin/includes/file.php';

    $cached_root = get_transient('xep_sync_repo_root');
    $cached_dir  = get_transient('xep_sync_temp_dir');
    if ($cached_root && is_dir($cached_root) && $cached_dir && is_dir($cached_dir)) {
        // Reuse existing extracted repo
        $repo_root = $cached_root;
        $temp_dir  = $cached_dir;
    } else {
        $temp_zip = download_url(XEPMARKET2_GITHUB_PLUGINS_REPO, 180);
        ...
        // Cache for 10 minutes so retries don't re-download
