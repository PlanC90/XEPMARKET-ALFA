...
function xepmarket2_ajax_prepare_plugins_sync() {
    ob_start(); // Prevent PHP warnings from breaking the JSON response
    error_log("[OmniXEP Sync] AJAX Prep starting...");
    check_ajax_referer(XEPMARKET2_GITHUB_SYNC_NONCE_ACTION, 'nonce');
    if (!current_user_can('install_plugins')) {
        error_log("[OmniXEP Sync] Permission denied.");
        ob_end_clean();
        wp_send_json_error(['message' => 'Permission denied.']);
    }
    ...
    @ini_set('memory_limit', '1024M'); // Maximize memory for PclZip fallback
    error_log("[OmniXEP Sync] Memory limit set, initializing filesystem...");
    
    // Force direct filesystem access to prevent FTP credential prompts on remote servers
    ...
    if (!$wp_filesystem) {
        error_log("[OmniXEP Sync] Filesystem init failed.");
        ob_end_clean();
        wp_send_json_error(['message' => 'Filesystem init failed. Please check folder permissions.']);
    }

    // 1. Download
    error_log("[OmniXEP Sync] Downloading: " . XEPMARKET2_GITHUB_PLUGINS_REPO);
    require_once ABSPATH . 'wp-admin/includes/file.php';
    
    // Disable SSL verification for this download to avoid issues on local environments
    ...
    $temp_zip = download_url(XEPMARKET2_GITHUB_PLUGINS_REPO, 300); // 5 minutes timeout
    ...
    if (!$wp_filesystem->mkdir($temp_dir)) {
        @unlink($temp_zip);
        ob_end_clean();
        wp_send_json_error(['message' => 'Could not create temp dir. Please check folder permissions.']);
    }
...
function xepmarket2_ajax_install_plugin_step() {
    ...
    if ($slug && isset(XEPMARKET2_OFFICIAL_PLUGIN_URLS[$slug])) {
        ...
        add_filter('http_request_args', function($args, $url) {
            if (strpos($url, 'downloads.wordpress.org') !== false || strpos($url, 'github.com') !== false) {
                $args['sslverify'] = false;
            }
            return $args;
        }, 10, 2);

        $temp_zip = download_url($url, 300); // 5 minutes timeout
    ...
    // ROBUSTNESS: Use directory move instead of file-by-file copy for performance
    if ($wp_filesystem->move($source, $dest, true)) {
        ob_end_clean();
        wp_send_json_success(['slug' => $slug, 'method' => 'move']);
    } else {
        // Fallback to recursive copy if move fails
        $errors = [];
        $success = xepmarket2_copy_plugin_dir($source, $dest, $errors);
        if ($success) {
            ob_end_clean();
            wp_send_json_success(['slug' => $slug, 'method' => 'copy']);
        } else {
            $err_msg = !empty($errors) ? implode('; ', $errors) : 'Unknown copy error';
