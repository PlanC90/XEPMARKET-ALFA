...
class XepMarket_Theme_Updater
{
    ...
    public function rename_github_folder($source, $remote_source, $upgrader, $hook_extra)
    {
        ...
        // Allow renaming for both updates and fresh installs (ZIP uploads)
        if (!isset($hook_extra['action']) || !in_array($hook_extra['action'], ['update', 'install'])) {
            return $source;
        }

        // If it's an update, ensure it's for our theme
        if ($hook_extra['action'] === 'update') {
            if (!isset($hook_extra['theme']) || ($hook_extra['theme'] !== $this->theme_slug && strpos($hook_extra['theme'], 'PlanC90-XEPMARKET-ALFA') !== 0)) {
                return $source;
            }
        }
        ...
        if ($source_files) {
            foreach ($source_files as $file) {
                // Determine if this looks like our GitHub output folder
                ...
                if (isset($file['type']) && $file['type'] === 'd' && $is_github_folder) {
                    ...
                    if ($wp_filesystem->exists(trailingslashit($potential_theme_dir) . 'style.css')) {
                        // Found our theme! We MUST physically rename the folder here inside the temp directory,
                        // because WP uses basename() of whatever we return to set the final destination folder.
                        $new_dir = trailingslashit($source) . $this->theme_slug;
                        
                        // If it's already properly named, just return it
                        if ($potential_theme_dir === $new_dir) {
                            return trailingslashit($new_dir);
                        }
                        
                        // Delete the target temp directory if it somehow exists
                        if ($wp_filesystem->exists($new_dir)) {
                            $wp_filesystem->delete($new_dir, true);
                        }

                        // Rename the folder from "PlanC90-XEPMARKET-ALFA-xxx" to "XEPMARKET-ALFA"
                        if ($wp_filesystem->move($potential_theme_dir, $new_dir)) {
                            return trailingslashit($new_dir);
                        } else {
                            // If move fails, return the original so WP doesn't completely break
