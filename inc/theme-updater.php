...
class XepMarket_Theme_Updater
{
    ...
    public function rename_github_folder($source, $remote_source, $upgrader, $hook_extra)
    {
        ...
        // Only act on theme updates/installs
        if (!($upgrader instanceof Theme_Upgrader)) {
            return $source;
        }

        // Check if source folder name contains our repo identifiers
        // WordPress gives us $source as the full path to the extracted folder,
        // e.g. /tmp/xxx/PlanC90-XEPMARKET-ALFA-abc123/
        ...
        if ($source_basename === 'XEPMARKET-ALFA') {
            return $source;
        }
        ...
        // Verify this is really our theme by checking for style.css
        if (!$wp_filesystem->exists(trailingslashit($source) . 'style.css')) {
            return $source;
        }

        // Build the new path: same parent directory, but with correct folder name
        $parent_dir = dirname(rtrim($source, '/\\'));
        $new_source = trailingslashit($parent_dir) . 'XEPMARKET-ALFA';
        ...
        // Rename: "PlanC90-XEPMARKET-ALFA-abc123" → "XEPMARKET-ALFA"
        if ($wp_filesystem->move(rtrim($source, '/\\'), $new_source)) {
            error_log('[XEP Updater] Renamed theme folder from "' . $source_basename . '" to "XEPMARKET-ALFA"');
            return trailingslashit($new_source);
        }

        // Move failed — log and return original to avoid breaking the update
        error_log('[XEP Updater] WARNING: Could not rename "' . $source_basename . '" to "XEPMARKET-ALFA"');
        return $source;
    }
}
